...
class Config:
    ...
    def __post_init__(self):
        if self.upload_dir is None:
            self.upload_dir = self.base_dir.parent / "cheque-processing-platform" / "Upload"
        if self.output_dir is None:
            self.output_dir = self.base_dir.parent / "cheque-processing-platform" / "Output"
        if self.model_path is None:
            self.model_path = self.base_dir.parent / "cheque-processing-platform" / "models" / "yolov8s.pt"
